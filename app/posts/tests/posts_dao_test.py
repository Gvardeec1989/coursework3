...
class TestPostsDao:
    ...
    def test_get_one_check_type_has_correct_pk(self, posts_dao, post_pk):
        ...
        assert post["pk"] == post_pk, "Номер поста не соответсвует номеру запрошенного"
    ...
    def test_get_posts_by_user(self, posts_dao, poster_name, post_pks_correct):
        ...
        assert post_pks == post_pks_correct, "Проверьте имя пользователя"
    ...
    def test_search_for_post(self, posts_dao, query, post_pks_correct):
        posts = posts_dao.search(query)
        post_pks = set()
        for post in posts:
            post_pks.add(post["pk"])

        assert post_pks == post_pks_correct, "Поиск не работает"
